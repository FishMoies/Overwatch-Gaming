    ...
    <div class="modal-content">
      ...
      <form @submit.prevent="handleSubmit" class="modal-form">
        <div class="form-group">
          <label>战队完整名称</label>
          <input type="text" v-model="name" required placeholder="请输入战队完整名称（包含#号）" class="form-input" />
          <p class="hint">例如：我的战队#1234</p>
        </div>
        <div v-if="message" class="msg" :class="{ error: isError }">{{ message }}</div>
        <div class="modal-actions">
          <button type="button" class="btn cancel" @click="$emit('close')">取消</button>
          <button type="submit" class="btn confirm">加入战队</button>
        </div>
      </form>
    </div>
  </div>
